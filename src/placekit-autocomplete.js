...
export default function placekitAutocomplete(
  ...
  if (!input) {
    ...
    input.tagName !== 'INPUT' ||
    !['text', 'search'].includes(input.getAttribute('type'))
  ) {
  ...
  const options = {
    ...
    format: {
      flag: (countrycode) =>
        `<img class="pka-flag" src="https://flagcdn.com/64x48/${countrycode?.toLowerCase()}.png" alt="${countrycode}" loading="lazy" />`,
      icon: (name, label) =>
  ...
  panel.innerHTML = `
    <div class="pka-panel-loading" role="progressbar" aria-hidden="true">${options.format.icon(
      'loading',
    )}</div>
    <div class="pka-panel-suggestions" role="listbox" aria-label="${
      options.format.suggestions
    }"></div>
    <div class="pka-panel-footer">
      <div class="pka-panel-country">
        <input type="checkbox" id="pka-panel-country-mode" class="pka-sr-only" />
        <label for="pka-panel-country-mode" class="pka-panel-country-open" aria-label="${
          options.format.changeCountry
        }">
        </label>
  ...
  function clearActive() {
    panel
      ...
      .forEach((node) => node.classList.remove('pka-active'));
  }
    ...
    pk.search(query, {
      ...
      .then(({ results }) => {
        ...
        <div class="pka-panel-suggestion" role="option" tabindex="-1" aria-selected="false">
          ${
            flagTypes.includes(item.type)
              ? options.format.flag(item.countrycode)
              : options.format.icon(item.type || 'pin', item.type)
          }
          ...
          <button type="button" class="pka-panel-suggestion-action" aria-label="${
            options.format.applySuggestion
          }" />
        </div>
      `,
                )
        ...
        <div class="pka-panel-suggestion" role="option" tabindex="-1" aria-selected="false" aria-disabled="true">
          ...
          <span class="pka-panel-suggestion-label">
            <span class="pka-panel-suggestion-label-name">
              ${
                options.format.noResults?.call
                  ? options.format.noResults(query)
                  : options.format.noResults
              }
            </span>
          </span>
        </div>
  ...
  function setCountry(item) {
    panel.querySelector('.pka-panel-country-open').innerHTML =
      item === null
        ? ''
        : `
      ${options.format.flag(item.countrycode)}
  ...
  function toggleCountryMode(bool) {
    countryMode.checked =
      !countryMode.disabled && (typeof bool === 'undefined' ? !countryMode.checked : bool);
